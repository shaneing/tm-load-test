...
type AggregateStats struct {
	...
	TotalTxsInBlock  int     // The total number of transactions sent.
	...
	AvgTxInBlockRate float64 // The rate at which transactions were submitted (tx/sec).
...
func (s *AggregateStats) Compute() {
	...
	if s.TotalTimeSeconds > 0.0 {
		...
		s.AvgTxInBlockRate = float64(s.TotalTxsInBlock) / s.TotalTimeSeconds
	}
}
...
func writeAggregateStats(filename string, stats AggregateStats) error {
	...
	records := [][]string{
		...
		{"total_txs_in_block", fmt.Sprintf("%d", stats.TotalTxsInBlock), "count in block"},
		{"avg_tx_in_block_rate", fmt.Sprintf("%.6f", stats.AvgTxInBlockRate), "transactions per second in block"},
	}
